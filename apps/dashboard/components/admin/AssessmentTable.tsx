...
export default function AssessmentTable({ rows, showUser = false }: { rows: AssessmentRow[]; showUser?: boolean }) {
  ...
  columns.push(
    { title: 'Instrumen', width: 100, render: r => <Text style={ui.text}>{r.instrument_type}</Text> },
    { title: 'Skor', width: 65, render: r => <Text style={[ui.text, { fontWeight: '700' }]}>{r.score}</Text> },
    { title: 'Keparahan tercatat', width: 145, render: r => <Badge value={r.severity} /> },
    { title: 'Tanggal', width: 125, render: r => <Text style={ui.muted}>{formatDate(r.taken_at)}</Text> },
  );
  if (showUser) columns.push({ title: 'Tindakan', width: 85, render: r => <Button label="Tinjau" tone="quiet" onPress={() => router.push(('/students/' + r.user_id) as Href)} /> });

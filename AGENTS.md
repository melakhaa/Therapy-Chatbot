...
TypeScript packages. Supabase (local Docker) provides Postgres, Auth, and vector storage; Ollama
provides local embeddings and the LLM.

## Tech stack

**Languages / runtimes**: Python 3.12 · TypeScript 5.9 · Node.js 20+ · SQL (PostgreSQL 17)

**Backend**: FastAPI · Uvicorn · Pydantic v2 · python-dotenv · cryptography (Fernet) · Supabase Python client
...
**Data / platform**: Supabase (Postgres, Auth) · Supabase CLI · Docker · PostgreSQL 17 ·
...
react-native-screens · react-native-svg · react-native-chart-kit · `@supabase/supabase-js` ·
...
| Data | Supabase platform + clients | [docs/supabase-conventions.md](docs/supabase-conventions.md) |
...
| Infra | Docker (local Supabase) | [docs/docker-conventions.md](docs/docker-conventions.md) |
...
- `apps/backend/docs/schema.sql` is an early draft (uses `profiles`/`operator`); the running schema
  uses `users` and roles `mahasiswa | konselor | admin | pemangku_jabatan`.
